...
export class XuatnhaptonsService {
  ...
  async getXuatnhaptonByid(id: any) {
    try {
      ...
      const response = await fetch(`${environment.APIURL}/xuatnhaptons/${id}`, options);
      if (!response.ok) {
        if (response.status === 401) {
          const result  = JSON.stringify({ code:response.status,title:'Vui lòng đăng nhập lại' })
          this.router.navigate(['/errorserver'], { queryParams: {data:result}});
  ...
  async updateOneXuatnhapton(dulieu: any) {
    try {
      const options = {
        ...
        const response = await fetch(`${environment.APIURL}/xuatnhaptons/${dulieu.id}`, options);
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  async DeleteXuatnhapton(item:any) {    
    try {
        const options = {
          ...
          const response = await fetch(`${environment.APIURL}/xuatnhaptons/${item.id}`, options);
          if (!response.ok) {
            if (response.status === 401) {
              const result  = JSON.stringify({ code:response.status,title:'Vui lòng đăng nhập lại' })
              this.router.navigate(['/errorserver'], { queryParams: {data:result}});
